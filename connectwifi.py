...
p = subprocess.run("networksetup -setairportnetwork en0 TELLO-EFFDE7", stdout=subprocess.PIPE, shell=True)
me = tello.Tello()
try:
    start0 = time.time()
    me.connect()
    ...
    while True:
        ...
        ret, img0 = cap.read()  # read frames
        if time.time() - start0 > 2:
            for i in range(3):
                try:
                    absdif[0] += abs(prbr[i] - brightness(img0)[i])
                except:
                    pass
            if absdif[0] < 15:
                for i in range(3):
                    prbr[i] = brightness(img0)[i]

            print(brightness(img0))
...
while True:
    # capture video
    print("video about to get captured")
    cap = me.get_video_capture()
    print("video got captured")
    # cap = cv2.VideoCapture(0)
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 360)  # 1280) #maybe change to 360
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 240)  # 720) #maybe change to 240
    print("video set dimensions")
    ret, frame = cap.read()  # read frames
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)  # convert to gray img
    corners, ids, rejected = aruco.detectMarkers(image=gray, dictionary=aruco_dict,parameters=parameters)  # , cameraMatrix = camera_martix, distCoeff= camera_distortion) #detect arucos
    print("about to go in if sattment of finding auroc")
    if ids != None and ids[0] == id_to_find:
        # ret = [rvec, tvec, ?]
        # array of rotation and position of each marker (1 in our case)
        print("about to arucoestimate")
        ret = aruco.estimatePoseSingleMarkers(corners, marker_size, camera_martix, camera_distortion)
        # we only need 1st
        print("about to find tvecs")
...
'''
source: https://www.the-analytics.club/python-shell-commands#:~:text=If%20you%20need%20to%20execute,arguments%20or%20producing%20text%20output.

s = subprocess.run("/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport -s", stdout=subprocess.PIPE, shell=True)



source: https://sharmank.medium.com/how-to-connect-to-wifi-using-mac-os-command-line-3a76c2e6669c
/System/Library/PrivateFrameworks/Apple80211.framework/Versions/Current/Resources/airport -s

networksetup -setairportnetwork en0 <SSID_OF_NETWORK> <PASSWORD>

networksetup -setairportnetwork en0 TELLO-EFFDE7
'''